...
import { User, Mail, Shield, Activity, Save, X, AlertTriangle } from 'lucide-vue-next';
  ...
  <AppLayout>
    <div class="db">
      ...
      <div class="db__header">
        <div>
          <h1 class="db__titulo">Editar Usuario</h1>
          <p class="db__sub">Modifica la información y permisos del usuario seleccionado.</p>
        </div>
      </div>
      ...
      <div class="inferior">
          ...
          <form class="formulario" @submit.prevent="submit" novalidate>
            ...
            <div class="campos-fila">
              ...
              <div class="campo campo--mitad" :class="{'has-error': form.errors.nombre}">
                ...
                <div class="input-grupo">
                  <User class="input-ico" />
                  <input type="text" class="input-text" v-model="form.nombre" />
                </div>
              ...
              <div class="campo campo--mitad" :class="{'has-error': form.errors.apellido}">
                ...
                <div class="input-grupo">
                  <User class="input-ico" />
                  <input type="text" class="input-text" v-model="form.apellido" />
                </div>
            ...
            <div class="campo" :class="{'has-error': form.errors.correo}">
              ...
              <div class="input-grupo">
                ...
                <input type="email" class="input-text" v-model="form.correo" />
              </div>
            ...
            <div class="alerta alerta--error mt-4" v-if="Object.keys(form.errors).length > 0">
              <AlertTriangle class="alerta__ico" /><span>Por favor revisa los errores en el formulario.</span>
            </div>
        ...
        <div class="panel panel--chico panel-info">
          <div class="panel__head">
            <h2 class="panel__titulo">Detalles del Registro</h2>
          </div>
          <div class="info-lista">
            <div class="info-item">
              <span class="info-label">Fecha de creación</span>
              <span class="info-valor">{{ formatDate(usuario.created_at) }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">Última modificación</span>
              <span class="info-valor">{{ formatDate(usuario.updated_at) }}</span>
            </div>
            <div class="info-item">
              <span class="info-label">Modificado por</span>
              <span class="info-valor">{{ usuario.modificador ? (usuario.modificador.nombre + ' ' + usuario.modificador.apellido) : 'Sistema / N/A' }}</span>
            </div>
          </div>
          <hr class="divider" />
          <div class="alerta alerta--info">
            <AlertTriangle class="alerta__ico" />
            <span style="font-size: 11px;">Al desactivar una cuenta, el usuario perderá acceso inmediato al sistema.</span>
          </div>
        </div>
...
@media (max-width:1100px) {
  ...
  .panel-info { order: -1; } /* Sube la info arriba en móviles */
}
